...
carto workflows create [json]
  --file <path>               # Read JSON from file (or pipe via stdin)
  --connection <name>         # Connection name (required for diagram JSON)
```
...
**Note**: This command is experimental. The `--connection` flag is required when uploading a workflow diagram JSON (to resolve the connection ID).

**Examples**:
```bash
carto workflows create --file workflow.json --connection my-bigquery
cat workflow.json | carto workflows create --connection carto_dw
```
